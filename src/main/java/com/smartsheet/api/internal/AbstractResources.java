 ...
 * 
 * Thread Safety:
 * This class is thread safe because it is immutable and the underlying SmartsheetImpl is thread safe.
*/
...
*/
private static final Map<Integer, Class<? extends SmartsheetRestException>> STATUS_CODE_TO_EXCEPTION_CLASS = null;// FIXME: what should this be?



// FIXME: only use the ErrorCode enum instead of the hashmap
public enum ErrorCode{
	INVALID_REQUEST(400, InvalidRequestException.class),
	AUTHORIZATION_EXCEPTION(401, AuthorizationException.class);
 ...
 * 
 * Implementation:
 * STATUS_CODE_TO_EXCEPTION_CLASS = new HashMap<String, String>();
 * STATUS_CODE_TO_EXCEPTION_CLASS.put(400, InvalidRequestException.class);
 * STATUS_CODE_TO_EXCEPTION_CLASS.put(401, AuthorizationException.class);
 * STATUS_CODE_TO_EXCEPTION_CLASS.put(403, AuthorizationException.class);
 * STATUS_CODE_TO_EXCEPTION_CLASS.put(404, ResourceNotFoundException.class);
 * STATUS_CODE_TO_EXCEPTION_CLASS.put(405, InvalidRequestException.class);
 * STATUS_CODE_TO_EXCEPTION_CLASS.put(500, SmartsheetRestException.class);
 ...
 * 
 * Parameters:
 * - path : the relative path of the resource collections
 * - objectClass : the resource object class
 ...
 * switch (response.getStatusCode()) {
 * case 200:
 *     return this.smartsheet.getJsonSerializer().deserializeResult(objectClass, response.getEntity().getContent()).getResult();
 * default:
 *     handleError(response);
 * }
 * @param path 
 ...
 * switch (response.getStatusCode()) {
 * case 200:
 *     return this.smartsheet.getJsonSerializer().deserializeList(objectClass, response.getEntity().getContent());
 * default:
 *     handleError(response);
 * }
 * @param path 
 ...
 * Parameters:
 * - path : the relative path of the resource collections
 * - objectToPost : the object to post
 ...
 * switch (response.getStatusCode()) {
 * case 200:
 *     return this.smartsheet.getJsonSerializer().deserializeListResult(objectClassToReceive, response.getEntity().getContent()).getResult();
 * default:
 *     handleError(response);
 * }
 * @param path 
 ...
 * 
 * ByteArrayOutputStream baos = new ByteArrayOutputStream();
 * this.smartsheet.getJsonSerializer().serialize(objectToPut, baos);
 * HttpEntity entity = new HttpEntity();
 * entity.setContentType("application/json");
 * entity.setContent(new ByteArrayInputStream(baos.toByteArray()));
 * entity.setContentLength(baos.size());
 ...
 * 
 * Exceptions:
 * Any exception shall be propagated since it's a private method.
 * 
 * Implementation:
 ...
 * Class<? extends SmartsheetRestException> exceptionClass = STATUS_CODE_TO_EXCEPTION_CLASS.get(response.getStatusCode());
 * if (exceptionClass == null) {
 *     exceptionClass = SmartsheetRestException.class;
 * }
 * throw (SmartsheetRestException) exceptionClass.getConstructor(Error.class).newInstance(error);
 * @param response 
*/
