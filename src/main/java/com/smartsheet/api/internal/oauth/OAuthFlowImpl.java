...
public class OAuthFlowImpl implements OAuthFlow {
     ...
     * 
     * Exceptions:
     * - IllegalArgumentException : if scopes is null/empty
     * 
     * Implementation:
     ...
     * 
     * Parameters:
     * - authorizationResponseURL : the authorization response URL
     * 
     * Returns:
     ...
     * - InvalidOAuthGrantException : if the authorization code or refresh token is invalid or expired, the redirect_uri does not match, or the hash value does not match the client secret and/or code
     * - UnsupportedOAuthGrantTypeException : if the grant type is invalid (note that this won't really happen in current implementation)
     ...
     *   .append(clientId)
     *   .append("&code=")
     *   .append(authorizationResult.getCode())
     *   .append("&redirect_uri=")
     *   .append(URLEncoder.encode(redirectURL, "utf-8"))
     *   .append("&hash=")
     *   .append(hash);
     ...
     * 
     * Parameters:
     * - url : the URL (with request parameters) from which the token will be requested
     * 
     * Returns:
     * the token.
     * 
     ...
     *     token.setExpiresInSeconds(map.get("expires_in"));
     *     return token;
     * } else {
     *     String errorType = map.get("error");
